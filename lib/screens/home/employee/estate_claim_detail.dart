...
class EstateClaimDetail extends StatefulWidget {
  ...
  EstateClaimDetail(this.claim_id);

  @override
  _EstateClaimDetailState createState() => _EstateClaimDetailState();
}

class _EstateClaimDetailState extends State<EstateClaimDetail> {
  Constants constants;
  UIUpdates uiUpdates;
  ...
  void GetEstateClaimDetail() async{
    try {
      ...
      var url = constants.getApiBaseURL() + constants.buildApiUrl(
          ...
          additionalPath: widget.claim_id);
        ...
        try {
          ...
          if (code == "1" || codeValue == 1) {
            ...
            if(data != null) {
              ...
              claim_location= data["claim_location"]?.toString() ?? "-";
              claim_quota= data["claim_quota"]?.toString() ?? "-";
              claim_dated= data["claim_dated"]?.toString() ?? "-";
              claim_abode= data["claim_abode"]?.toString() ?? "-";
              claim_number= data["claim_number"]?.toString() ?? "-";
              claim_floor= data["claim_floor"]?.toString() ?? "-";
              claim_street= data["claim_street"]?.toString() ?? "-";
              claim_block= data["claim_block"]?.toString() ?? "-";
              claim_impound= data["claim_impound"]?.toString() ?? "-";
